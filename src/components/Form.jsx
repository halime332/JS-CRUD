...
const Form = ({ setTodos }) => {
    ...
    const handleSubmit = (e) => {
        ...
        // API'ya gidicek nesneyi hazırla
        const newTodo = {
            title: text,
            status,
        ...
        api
            .post("/todos", newTodo)
            //  oluşturulan nesneyi state kaydet(arayüze eklenir)
            .then((res) => {
                setTodos((todos) => [res.data, ...todos])
                toast.success("Todo oluşturuldu");
            })
            //istek başarısız olursa uyarıo gönder
            .catch((err) => toast.error("Bir sorun oluştu!!"));


        // formun sıfırlanması için
        e.target.reset();
    ...
    return (
        <form onSubmit={handleSubmit} className="d-flex justify-content-center gap-3 my-5">
            <input placeholder="ör:react projesi yap" className="form-control shadow" />
